...
object Definitions {
  ...
  private def chooseNotRewritten: Nothing = sys.error("``choose'' was not rewritten by the synthesis plugin")
  ...
  private def extractorNotRewritten: Nothing = sys.error("Illegal use of an arithmetic extractor (use it at top-level only, without guards, and make sure you run scalac with the synthesis plugin).")
  object + {
